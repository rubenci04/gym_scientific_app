...
class SeedDataService {
  static Future<void> initializeExercises() async {
    ...
    if (exerciseBox.isEmpty) {
      ...
      debugPrint(
        "✅ Base de datos completada: ${allExercises.length} ejercicios cargados.",
      );
    } else {
      debugPrint(
        "ℹ️ Base de datos ya inicializada (${exerciseBox.length} ejercicios).",
      );
    }
  }
  ...
  static Future<void> updateExerciseDatabase() async {
    ...
    for (var exercise in allExercises) {
      if (exerciseBox.containsKey(exercise.id)) {
        // Actualizar ejercicio existente (por si cambiamos descripción o tips)
        await exerciseBox.put(exercise.id, exercise);
        updated++;
      } else {
    ...
    debugPrint(
      "✅ Actualización completa: $added nuevos, $updated actualizados.",
    );
  }
}
